...
for(int y=0; y<MAX; y++){
	...
	ptr[y].valor = 0;
	ptr[y].ponteiroAnterior =0;
	ptr[y].ponteiroProximo = 0;
}
...
do{
		...
		if(total_lista <tamanho){
			situacao=0;
		}
		else if(total_lista ==tamanho){
			situacao=1;
		}
		if (situacao==1){
			...
			system("cls");
			printf("%c-----------------------------------------------------------------------%c\n", 218, 191);
			printf("           A LISTA ESTA CHEIA, EXCLUA VALORES PARA DESOCUPA-LA.   ");
			printf(" \n");
			printf("%c-----------------------------------------------------------------------%c\n", 218, 191);
	...
	do{
			...
			ptr[gx-1].ponteiroAnterior = lt[gx-1].valor;
			
			ptr[gx-1].ponteiroProximo = lt[gx-1].valor;
